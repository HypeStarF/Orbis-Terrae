...
This directory contains the permanent synthetic reference atlas for Phase 1 Step 4. It is deliberately
small enough to inspect manually while using the same manifest, OTAT tile reader, corruption checks,
path resolution, cache and geographic samplers as a future real atlas.

The fixture is synthetic project-owned test data. It is not derived from a real GIS dataset and does
...
There are four binary tiles for each layer. The checked-in files can be opened directly by
`AtlasDirectory`.
